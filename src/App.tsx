...
import { curRunningPIDContext, FREE_LIST_ADDRESS, machineReducer } from "./simulation/machine-reducer";
...
export function App() {
    ...
    const freeList = useMemo(() => {
        return getFreeList(machineState);
    }, [machineState]);

    const activePageTablesBases = useMemo(() => {
        return getActivePageTablesBases(machineState);
    }, [machineState]);

    const allPageTables = useMemo(() => {
        return getAllPageTables(machineState);
    }, [machineState]);

    const allProcessPages = useMemo(() => {
        // console.log("current allProcessPages:", allProcessPages);
        return getAllProcessPages(machineState);
    }, [machineState]);
    ...
    <curRunningPIDContext.Provider value={{curRunningPID, setCurRunningPID}}>
        <SidebarProvider>
            ...
            <div className="py-10  pl-1 w-full h-full flex flex-3 flex items-start gap-4">
                ...
                machineState={machineState}></MemoryCard>
                <VirtualMemory machineState={machineState} activePageTablesBases={activePageTablesBases}> </VirtualMemory>
            </div>
        </SidebarProvider>
